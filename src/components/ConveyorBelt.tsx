...
export const SLOT_COORDS: Record<number, { x: number; y: number }> = {
  0: { x: 12, y: 88 },  // Bottom Left
  1: { x: 40, y: 88 },  // Bottom Center-Left
  2: { x: 64, y: 88 },  // Bottom Center-Right
  3: { x: 88, y: 88 },  // Bottom Right
  4: { x: 94, y: 62 },  // Right Bottom
  5: { x: 94, y: 36 },  // Right Top
  6: { x: 88, y: 12 },  // Top Right
  7: { x: 64, y: 12 },  // Top Center
  8: { x: 40, y: 12 },  // Top Center-Left
  9: { x: 12, y: 12 },  // Top Left
  10: { x: 6, y: 36 },  // Left Top
  11: { x: 6, y: 62 },  // Left Bottom
};

interface ConveyorBeltProps {
  plates: Plate[];
  onPlateClick?: (plate: Plate) => void;
  highlightedSlots?: number[]; // indices of flash/warning slots
  lastTickTime: number;
  beltSpeed: number;
...
export const ConveyorBelt: React.FC<ConveyorBeltProps> = ({
  ...
  // Map index to plates on the belt
  ...
  if (variant === 'tweak') {
    return (
      <div 
        className="relative w-full aspect-[1.12/1] bg-[#070a0f] border-4 border-[#1e293b] rounded-2xl shadow-2xl p-3 overflow-visible select-none"
        style={{
          boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.7), inset 0 1px 3px rgba(255, 255, 255, 0.05)',
        }}
      >
        {/* Monospace Circuit Loop Track */}
        <div className="absolute inset-1.5 border-[2px] border-[#1e293b] rounded-xl overflow-hidden bg-[#090d14] shadow-inner">
          ...
          {/* Central Loop Control Processor (Center Island) */}
        ...
        {/* Render Slot Dots / Guides */}
        {Object.entries(SLOT_COORDS).map(([idxStr, coord]) => {
          const idx = parseInt(idxStr, 10);
          const isHighlighted = highlightedSlots.includes(idx);
          const isOccupied = !!beltSlots[idx];
        ...
        {/* Plates overlaying their respective slots */}
        <AnimatePresence>
          {safePlates.map((plate) => {
            const currIdx = plate.currentSlot;
            const coordCurr = SLOT_COORDS[currIdx];
            ...
            return (
              <motion.div
                ...
                transition={{
                  type: 'tween',
                  ease: 'linear',
                  duration: beltSpeed / 1000,
                }}
                style={{
                  zIndex: 25,
                }}
                className="absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-transform hover:scale-110 duration-200"
              >
                <SushiPlate
  ...
  if (variant === 'zen') {
    return (
      <div 
        className="relative w-full aspect-[1.12/1] bg-[#edd2b6] border-[6px] border-[#9c6644] rounded-[48px] shadow-2xl p-3 overflow-visible select-none cursor-pointer active:scale-[0.98] active:brightness-95 transition-all duration-150"
        style={{
          boxShadow: '0 20px 40px -15px rgba(66, 35, 12, 0.5), inset 0 2px 8px rgba(255, 255, 255, 0.4)',
        }}
        onClick={() => onBeltTap?.()}
      >
        {/* Conveyor Belt Track Texture - Zen Wooden Deck Tracks */}
        <div className="absolute inset-1 border-[4px] border-[#7f5539] rounded-[42px] overflow-hidden bg-[#e6ccb2] shadow-inner">
          
          {/* Continuous overlapping scale-like links forming rotating tracks! */}
          <div 
          ...
          {/* Serene Blue Water Pool ("Zen Garden Pond") */}
          <div 
          ...
          >
            ...
            {/* Floating Pink Cherry Blossoms (Sakura petals/blooms) */}
            <motion.div
            ...
            {/* Swimming Koi Fish (Slowly swimming in circles) */}
            <motion.div
              animate={{
                rotate: [0, 360],
              }}
            ...
            {/* Minimalist Water ripples on center */}
        ...
        {Object.entries(SLOT_COORDS).map(([idxStr, coord]) => {
          ...
          return (
            <div
              ...
              style={{ left: `${coord.x}%`, top: `${coord.y}%` }}
            >
              {/* Soft decorative wooden slot outline */}
              <div
            ...
            return (
              ...
              >
                <SushiPlate
                  variety={plate.variety}
                  count={plate.count}
                  size={38} // Balanced size to fit perfectly on the wider tracks without overlapping
                  onClick={() => onPlateClick?.(plate)}
                />
              </motion.div>
            );
  ...
  // Fallback to Classic Option 1 (Pristine Original)
  return (
    <div className="relative w-full aspect-[1.3/1] bg-[#1a1a1a] border-[8px] border-[#2d2d2d] rounded-[36px] shadow-2xl p-2.5 overflow-visible select-none">
      ...
      <div className="absolute inset-1.5 border-[4px] border-[#1d1d1d] rounded-[28px] overflow-hidden bg-[#121212] shadow-inner">
        {/* Subtle repeating vertical stripes to look like belt links - moving dynamically! */}
        <div 
        ...
        {/* Center Island / Chef Table inside the loop */}
        <div className="absolute inset-[15%] bg-[#1d1d1d] border-2 border-[#2c2c2c] rounded-[18px] shadow-lg flex flex-col items-center justify-center">
          <div className="text-center">
            {/* Visual logo for center island in fine Serif styling */}
      ...
      {Object.entries(SLOT_COORDS).map(([idxStr, coord]) => {
        ...
        return (
          <div
            ...
            style={{ left: `${coord.x}%`, top: `${coord.y}%` }}
          >
            {/* Soft indicator dot underneath the plates */}
            <div
             ...
             />
            {/* Small debug/visual slot indicator */}
